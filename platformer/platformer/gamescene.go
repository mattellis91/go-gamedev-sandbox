...
type GameScene struct {
	worldPlatformer *WorldPlatformer
}
...
func NewGameScene() *GameScene {
	g := &GameScene{
		worldPlatformer: NewWorldPlatformer(),
	}
	return g
}
...
func (world *WorldPlatformer) Init() {
	...
	gw := float64(ScreenWidth)
	gh := float64(ScreenHeight)
...
func (s *GameScene) Update(state *GameState) error {
	s.worldPlatformer.Update()
	return nil
}

func (s *GameScene) Draw(screen *ebiten.Image) {
	s.worldPlatformer.Draw(screen)
	return
}